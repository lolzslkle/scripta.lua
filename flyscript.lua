...
-- ตัวแปรที่เก็บข้อมูลผู้ใช้งาน
local playerPermissions = {
    ["AdminPlayer1"] = "admin", -- ผู้เล่นคนนี้มีสิทธิ์ทุกอย่าง
    ["Player2"] = "user", -- ผู้เล่นคนนี้จำกัดคำสั่ง
}

-- ตรวจสอบผู้เล่น
local function getPlayerRole(player)
    return playerPermissions[player.Name] or "user"
end

-- ฟังก์ชันการดำเนินการคำสั่ง
local function executeCommand(player, command)
    local role = getPlayerRole(player)
    
    -- กำหนดคำสั่งที่สามารถใช้งานได้
    ...
    -- หากผู้ใช้มีคำสั่งที่จำกัด
    if restrictedCommands[command] and role ~= "admin" then
        print(player.Name .. " tried to use a restricted command: " .. command)
        return "You do not have permission to use this command."
    end

    -- คำสั่งที่สามารถใช้งานได้
    if allowedCommands[command] then
        -- ทำการประมวลผลคำสั่งที่อนุญาต
        print(player.Name .. " executed command: " .. command)
        -- ทำการกระทำกับคำสั่งนั้น ๆ เช่น ฟังก์ชันการบิน, โหมดพระเจ้า, ฯลฯ
...
-- ตัวอย่างคำสั่งบิน (Fly)
local function flyPlayer(player)
    -- ตัวอย่างฟังก์ชันการบินสำหรับผู้เล่น
    -- ปรับพารามิเตอร์หรือเพิ่มฟังก์ชันบินให้เหมาะสม
    print(player.Name .. " is now flying!")
    -- เพิ่มฟังก์ชันบินที่นี่
end

-- รับคำสั่งจากผู้เล่น
local player = game.Players.LocalPlayer
local command = "fly" -- เปลี่ยนเป็นคำสั่งที่ผู้เล่นส่งมา

local result = executeCommand(player, command)
print(result)

-- ถ้าคำสั่งคือ "fly" ก็เรียกใช้ฟังก์ชันบิน
if command == "fly" then
    flyPlayer(player)
end
